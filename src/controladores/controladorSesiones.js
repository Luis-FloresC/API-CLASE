...
exports.guardar = async (req, res) => {
    ...
    if (!usuarios_id || !estado) {
        msj("No se enviaron los datos correctos",200,["Falta el Id del usuario"],res)
        //console.log("El id es obligatorio");
        //res.send("El id es obligatorio");
    } else {
        await modeloSesiones.create({...req.body})
            .then((data) => {
                console.log(data);
                var info =
                    "La Sesión se registro con éxito";
                res.send(info);
            })
            .catch((error) => {
                console.log(error);
...
exports.incioSesion = async (req, res, next)=> {
    ...
    if (!validacion.isEmpty)
    {
        ...
        {
            if(!BuscarUsuario.VerificarContrasenia(contrasena, BuscarUsuario.contrasena))
            {
                msj("El Usuario no existe o contraseña invalida", 200, [], res);
            }
            else
            {
                const BuscarPersona = await modeloPersona.findByPk(BuscarUsuario.personas_id);
                const DatosUsuario = {
                ...
                console.log(data)
                msj("Bienvenido, " + DatosUsuario.Nombre + " " + DatosUsuario.Apellido, 200, data, res);
            }
        }
    }
};
